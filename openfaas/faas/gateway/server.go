...
func main() {
	...
	exporter.StartServiceWatcher(*config.FunctionsProviderURL, metricsOptions, "func", time.Second * 5)
	...
	if config.DirectFunctions { // true: using k8s url resolver
		...
		log.Println("Direct functions enabled: Using kubernetes url dispatcher")
	} else { // false: using self-defined url resolver
		functionURLResolver = urlResolver
		functionURLTransformer = nilURLTransformer
		log.Println("Direct functions disabled: Using self-defined url dispatcher")
	}
	...
	if faasHandlers.QueuedProxy != nil {
		...
		log.Println("QueuedProxy enabled: Using async functions.")
	} else {
		log.Println("QueuedProxy disabled: Not using async functions.")
	}
