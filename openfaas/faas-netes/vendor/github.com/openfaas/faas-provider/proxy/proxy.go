...
func proxyRequest(w http.ResponseWriter, originalReq *http.Request, proxyClient *http.Client, resolver BaseURLResolver) {
	...
	log.Printf("MyDebug2 proxyRequest functionAddr %s",functionAddr)
	if resolveErr != nil {
	...
	log.Printf("LatencyRecord Function: %s Time: %v",functionName,time.Since(start))
	if errors != nil {
