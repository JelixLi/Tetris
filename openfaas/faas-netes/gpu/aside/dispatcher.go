...
func updateFunctionsLottery(curQps int32, funcDeployStatus *gTypes.FuncDeployStatus) {
	if curQps > funcDeployStatus.FuncPodMaxCapacity {
		...
		// log.Println("My Debug-1 updateFunctionsLottery")
		...
		// log.Println("My Debug-2 updateFunctionsLottery")
		...
		// log.Println("My Debug-3 updateFunctionsLottery")
		if curQps == 0 {
			for _, funcConfig := range funcDeployStatus.FuncPodConfigMap {
				if funcConfig.FuncPodType == "i" {
					repository.UpdateFuncPodLottery(funcDeployStatus.FunctionName, funcConfig.FuncPodName, 0)
			...
			// log.Printf("My Debug-4 %d", (int32(float64(funcDeployStatus.FuncPodMaxCapacity-funcDeployStatus.FuncPodMinCapacity)*PodLoadRate) + funcDeployStatus.FuncPodMinCapacity))
			// changePodForLowWorkload(funcDeployStatus, curQps)
			changePodForLowWorkloadNew(funcDeployStatus, curQps, (int32(float64(funcDeployStatus.FuncPodMaxCapacity-funcDeployStatus.FuncPodMinCapacity)*PodLoadRate) + funcDeployStatus.FuncPodMinCapacity))
		}
...
func changePodForLowWorkloadNew(funcDeployStatus *gTypes.FuncDeployStatus, curReq int32, targetReq int32) {
	...
	DeletedNameMap := make(map[uint]int32)

	n := uint(len(letter))
	delete_req := targetReq - curReq
	if delete_req <= 0 {
		return
	}
	for {
		// log.Println("delete debug")
		if delete_req > 0 {
			candidate_instance := uint(0)
			candidate_rmax := int32(-1)
			found := false
			for j = 0; j < n; j++ {
				if DeletedNameMap[j] == 1 {
					continue
				}
				// if (candidate_instance==-1) || (letter[j].ReqPerSecondMax < candidate_rmax) {
				// 	candidate_rmax = letter[j].ReqPerSecondMax
				// 	candidate_instance = j
				// }
				if (!found) || (letter[j].ReqPerSecondMax < candidate_rmax) {
					candidate_rmax = letter[j].ReqPerSecondMax
					candidate_instance = j
					found = true
				}
			}
			// if candidate_instance == -1 {
			// 	break
			// }
			if !found {
				break
			}
			if (delete_req - candidate_rmax) <= 0 {
				break
			}
			foundFlag = true
			DeletedNameMap[candidate_instance] = 1
			delete_req -= candidate_rmax
			minDeletedSumCap += letter[candidate_instance].ReqPerSecondMin
			maxDeletedSumCap += letter[candidate_instance].ReqPerSecondMax
			deletePodNameList = append(deletePodNameList, letter[candidate_instance].FuncPodName)
			remainPodNameList = append(remainPodNameList, letter[candidate_instance].FuncPodName)
		} else {
			break
		}
	}
...
//
//	for c := candidateNum; c > 0 && foundFlag == false; c-- {
