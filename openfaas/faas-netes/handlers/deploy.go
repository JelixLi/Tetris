...
const MonitorInterval = time.Second * 3

var FuncQpsPerInstanceTable = map[string]float64{
	"resnet-50-keras": 500,
}
...
func MakeDeployHandler(functionNamespace string, factory k8s.FunctionFactory, clientset *kubernetes.Clientset) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		qpsPerInstances := FuncQpsPerInstanceTable[request.Service]
		err = nil
		log.Printf("Make Deploy qpsPerInstances: %f", qpsPerInstances)
		...
		for i := *int32p(0); i < *initialReplicas; i++ {
			...
			repository.AddVirtualFuncPodConfig(request.Service)

		}
		// can be deleted
		go scheduler.CreatePreWarmPod(request.Service, namespace, qpsPerInstances, 1, clientset)
		go instanceScaleMonitor(request.Service, functionNamespace, clientset)
...
func instanceScaleMonitor(funcName string, namespace string, clientset *kubernetes.Clientset) {
	...
	readLock := repository.GetFuncScalingLockState(funcName)
	for {
		select {
		case <-ticker.C:
