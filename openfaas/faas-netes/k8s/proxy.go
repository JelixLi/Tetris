...
func (l *FunctionLookup) Resolve(name string) (url.URL, error) {
	/*
		...
		urlStr := fmt.Sprintf("http://%s:%d", serviceIP, watchdo  gPort)
...
func reqDispatcher(funcName string) string {
	...
	if funcStatus != nil {
		if funcStatus.FuncPodTotalLottery == 0 {
			...
			winner := rand.Intn(int(funcStatus.FuncPodMaxCapacity))
			// log.Printf("+++++++++++++++++++++++++++++++ winner=%d\n",winner)
			for _, v := range funcStatus.FuncSortedPodNameList {
				if v =="v" {
					continue
				}
				counter = counter + funcStatus.FuncPodConfigMap[v].ReqPerSecondMax
				// log.Printf("+++++++++++++++++++++++++++++++ counter > winnner %d %d\n", counter, winner)
				// log.Printf("FuncPodName %s\n", v)
				// log.Printf("FuncPodIp %s\n", funcStatus.FuncPodConfigMap[v].FuncPodIp)
				if counter >= int32(winner) {
			...
			rand_instance_id := rand.Intn(int(len(funcStatus.FuncSortedPodNameList)))
			if funcStatus.FuncSortedPodNameList[rand_instance_id] == "v" {
				rand_instance_id = rand.Intn(int(len(funcStatus.FuncSortedPodNameList)))
			}
			return funcStatus.FuncPodConfigMap[funcStatus.FuncSortedPodNameList[rand_instance_id]].FuncPodIp
		}
	}
	return ip
}
